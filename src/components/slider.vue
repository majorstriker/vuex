<template>
  <div class="v_slider">
      ...
      <ul v-if="dots" class="v_slider__dots">
          ...
          <li v-for="n in numDot" @click="setDot(n)" :class="{'active': n == numDotActive}" :key="n.index">
              <span></span>
          </li>
      </ul>
...
<style>
  ...
  .v_slider__item { min-height: 1px; float: left; -webkit-backface-visibility: hidden; }
  ...
  .v_slider__dots li span { width: 16px; height: 16px; background: #fff; border: 2px solid black; float: left;  border-radius: 50%; }
  .v_slider__dots li span:hover { background: #C4C4C4 ; }
  .v_slider__dots li.active span { background: #C4C4C4; }
  
  .v_slider__prev,
  .v_slider__next { outline: none; background:transparent; padding: 10px 15px;  font-size: 20px; opacity: 0.9; }
  .v_slider__prev:hover,
  .v_slider__next:hover { cursor: pointer; }
